...
nvcc -arch=sm_20 -O2 saxpy_cuda_timed_pinned.cu
*/
...
int main(int argc, char *argv[])
{
   ...
   if(error = cudaMallocHost((void **) &x_host, memsize))
{
      printf ("Error in cudaMallocHost %d\n", error);
      exit (error);
}

   if(error = cudaMallocHost((void **) &y_host, memsize))
{
      printf ("Error in cudaMallocHost %d\n", error);
      exit (error);
}

   if(error = cudaMallocHost((void **) &y_shadow, memsize))
{
      printf ("Error in cudaMallocHost %d\n", error);
      exit (error);
}

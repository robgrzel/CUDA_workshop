...
// Input 2D array:
float h_A[STRIDE][NMAX];
__device__ float d_A[STRIDE][NMAX];
...
for (kk=0; kk<NTESTS; kk++)
{
  ...
  for (int i=0; i<NMAX; i++)
    for (int j=0; j<STRIDE; j++)
      h_A[j][i] = (float)rand()/(float)RAND_MAX;
  ...
  if (Nblocks > deviceProp.maxGridSize[0])
    {
      printf ("Nblocks > maxGridSize!  %d  %d\n", Nblocks, deviceProp.maxGridSize[0]);
      exit (1);
    }
